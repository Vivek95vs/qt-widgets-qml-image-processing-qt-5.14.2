// collimationmanager.cpp - Complete fixed version
...
void CollimationManager::setCropRect(const QRectF &rect)
{
    ...
    if (m_cropRect != newRect) {
        ...
        qDebug() << "Crop rect updated to:" << m_cropRect;
    }
}
                                      ...
                                      qreal imageWidth, qreal imageHeight)
{
    ...
    qDebug() << "Start resize - handle:" << handleIndex
             << "startPos:" << startX << startY
             << "imageSize:" << imageWidth << imageHeight
             << "startRect:" << m_startRect;
}
...
void CollimationManager::updateResize(qreal currentX, qreal currentY)
{
    if (m_activeHandle < 0) return;
        ...
        qDebug() << "Resize update - delta:" << deltaX << deltaY << "dx dy:" << dx << dy;
    ...
    if (newRect.left() > newRect.right()) {
        qreal temp = newRect.left();
        newRect.setLeft(newRect.right());
        newRect.setRight(temp);
    }

    if (newRect.top() > newRect.bottom()) {
        qreal temp = newRect.top();
        newRect.setTop(newRect.bottom());
        newRect.setBottom(temp);
    }
    ...
    if (newRect.width() < 0.5) {
        if (newRect.left() <= 0) newRect.setRight(0.5);
        else newRect.setLeft(newRect.right() - 0.5);
    }
    if (newRect.height() < 0.5) {
        if (newRect.top() <= 0) newRect.setBottom(0.5);
        else newRect.setTop(newRect.bottom() - 0.5);
    }
...
{
    qDebug() << "End resize - final rect:" << m_cropRect;
    m_activeHandle = -1;
}
...
void CollimationManager::startMove(qreal startX, qreal startY, qreal imageWidth, qreal imageHeight)
{
    ...
    qDebug() << "Start move - startPos:" << startX << startY
             << "startRect:" << m_startRect;
}
...
void CollimationManager::updateMove(qreal currentX, qreal currentY)
{
    ...
    qDebug() << "Move update - delta:" << deltaX << deltaY
             << "move:" << moveX << moveY;
    ...
    if (newRect.right() > m_originalSize.width()) newRect.moveRight(m_originalSize.width());
    if (newRect.bottom() > m_originalSize.height()) newRect.moveBottom(m_originalSize.height());
...
{
    m_isMoving = false;
    qDebug() << "End move - final rect:" << m_cropRect;
}

void CollimationManager::startDraw(qreal startX, qreal startY, qreal imageWidth, qreal imageHeight)
{
    m_isDrawing = true;
    m_startX = startX;
    m_startY = startY;
    ...
    qDebug() << "Start draw - start:" << startX << startY;
}

void CollimationManager::updateDraw(qreal currentX, qreal currentY)
{
    ...
    qreal left = qMin(m_startX, currentX) * scaleX;
    qreal top = qMin(m_startY, currentY) * scaleY;
    qreal width = qAbs(currentX - m_startX) * scaleX;
    qreal height = qAbs(currentY - m_startY) * scaleY;

    qDebug() << "Draw update - rect:" << left << top << width << height;

    setCropRect(QRectF(left, top, width, height));
}

void CollimationManager::endDraw()
{
    m_isDrawing = false;
    qDebug() << "End draw - final rect:" << m_cropRect;
}
