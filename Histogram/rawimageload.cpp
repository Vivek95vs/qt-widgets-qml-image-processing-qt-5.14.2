...
RawImageLoad::RawImageLoad(QObject *parent)
    : QObject(parent), QQuickImageProvider(QQuickImageProvider::Image), m_imageLoaded(false), m_windowWidth(255), m_windowLevel(128), originalArr(nullptr) {
}
...
void RawImageLoad::loadImage() {
    ...
    if (!myImage.isNull()) {
        ...
        applyWindow(); // Apply initial windowing
...
void RawImageLoad::setWindowWidth(int width) {
    if (m_windowWidth != width) {
        m_windowWidth = width;
        applyWindow(); // Apply windowing with new WW
...
void RawImageLoad::setWindowLevel(int level) {
    if (m_windowLevel != level) {
        m_windowLevel = level;
        applyWindow(); // Apply windowing with new WL
...
void RawImageLoad::resetImage() {
    if (originalArr) {
        // Restore the original pixel values
        memcpy(arr, originalArr, 3072 * 3072);
...
QImage RawImageLoad::loadRawImage() {
    ...
    originalArr = new unsigned char[w * h]; // Store the original pixel values

    // Scale the 16-bit raw data to 8-bit [0, 255]
    for (int i = 0; i < w * h; i++) {
        arr[i] = static_cast<unsigned char>((Image[i] * 255) / 65535); // Scale to 8-bit
        originalArr[i] = arr[i]; // Store the original pixel values
    }
