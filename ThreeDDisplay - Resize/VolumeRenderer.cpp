...
VolumeRenderer::VolumeRenderer(QObject *parent) : QObject(parent) {
    m_reader = vtkSmartPointer<vtkDICOMReader>::New();
}

void VolumeRenderer::setRenderer(QObject* renderer) {
    if (!renderer) {
        qWarning() << "Received null render window!";
        return;
    }

    qDebug() << "Received object of type:" << renderer->metaObject()->className();

    // Cast to QQuickVTKRenderWindow (this is the window for rendering)
    QQuickVTKRenderWindow* vtkWindow = qobject_cast<QQuickVTKRenderWindow*>(renderer);
    if (!vtkWindow) {
        qWarning() << "Failed to cast to QQuickVTKRenderWindow!";
        return;
    }

    // Access vtkRenderWindow using the renderWindow() method
    vtkRenderWindow* renderWindow = vtkWindow->renderWindow();  // Use renderWindow() instead of getRenderWindow()

    if (!renderWindow) {
        qWarning() << "Failed to get vtkRenderWindow from QQuickVTKRenderWindow!";
        return;
    }

    // If vtkRenderWindow exists, you can get the vtkRenderer from it
    m_renderer = renderWindow->GetRenderers()->GetFirstRenderer();
    if (m_renderer) {
        qDebug() << "Renderer set successfully!";
    } else {
        qWarning() << "Failed to get vtkRenderer from vtkRenderWindow!";
    }
}


void VolumeRenderer::startRender(const QString& folderPath) {
...
void VolumeRenderer::runVolumePipeline(vtkImageData* data) {
    if (!m_renderer) {
