...
DicomHandler::DicomHandler(QObject* parent)
    ...
    , m_lowerThreshold(-1000)
    , m_upperThreshold(1000)
{
}
...
QImage DicomHandler::getCoronalSlice(int ySlice, int width, int height) {
    ...
    // Coronal/Frontal view - matches your QtWidgets code orientation
    // Your code shows: (ctDIM3-1-iRow) * (int)ctDIM1 + ctDIM1-1-iCol
    // This means: flip vertically (ctDIM3-1-iRow) and flip horizontally (ctDIM1-1-iCol)

    for (int z = 0; z < height; ++z) {           // Z dimension (depth)
        uchar* line = qimg.scanLine(z);
        for (int x = 0; x < width; ++x) {        // X dimension (width)
            Image3DType::IndexType idx;
            idx[0] = x;                // X coordinate
            idx[1] = ySlice;           // Y slice (fixed for coronal)
            idx[2] = z;                // Z coordinate

            double pixelValue = m_volume->GetPixel(idx);

            // Apply your orientation: flip horizontally and vertically
            int targetX = width - 1 - x;      // Flip horizontally
            int targetZ = height - 1 - z;     // Flip vertically

            uchar* targetLine = qimg.scanLine(targetZ);

            if (pixelValue <= lowerThreshold) {
                targetLine[targetX] = 0;
            } else if (pixelValue >= upperThreshold) {
                targetLine[targetX] = 255;
            } else {
                double normalized = ((pixelValue - lowerThreshold) * 255.0) / windowRange;
                targetLine[targetX] = static_cast<uchar>(qBound(0.0, normalized, 255.0));
            }
        }
    }
...
QImage DicomHandler::getSagittalSlice(int xSlice, int width, int height) {
    ...
    // Sagittal view - matches your QtWidgets code orientation
    // Your code shows: (ctDIM3-1-iRow) * (int)ctDIM2 + iCol
    // This means: flip vertically (ctDIM3-1-iRow), keep horizontal as is

    for (int z = 0; z < height; ++z) {           // Z dimension (depth)
        uchar* line = qimg.scanLine(z);
        for (int y = 0; y < width; ++y) {        // Y dimension (height)
            Image3DType::IndexType idx;
            ...
            // Apply your orientation: flip vertically only
            int targetZ = height - 1 - z;     // Flip vertically
            int targetY = y;                  // Keep horizontal as is

            uchar* targetLine = qimg.scanLine(targetZ);

            if (pixelValue <= lowerThreshold) {
                targetLine[targetY] = 0;
            } else if (pixelValue >= upperThreshold) {
                targetLine[targetY] = 255;
            } else {
                double normalized = ((pixelValue - lowerThreshold) * 255.0) / windowRange;
                targetLine[targetY] = static_cast<uchar>(qBound(0.0, normalized, 255.0));
            }
        }
    }
